 ...
 * 如果 buffer_ 空间足够 → 直接写入。
 * 如果 buffer_ 空间不足，但仍然可以扩容 → 先读入 extrabuf，再扩容并追加。
 * 读失败 → 设置错误码。
 **/
ssize_t Buffer::readFd(int fd, int *saveErrno)
{
    // 栈额外空间，用于从套接字往出读时，当buffer_暂时不够用时暂存数据，待buffer_重新分配足够空间后，在把数据交换给buffer_。
    char extrabuf[65536] = {0}; // 栈上内存空间 65536/1024 = 64KB，tcp单个报文最大接受窗口就是64kb
    ...
    */
	...
	// TCP 单个报文最大接收窗口 = 64KB → 合理设计接收缓冲区为 64KB。
    const int iovcnt = (writable < sizeof(extrabuf)) ? 2 : 1;
    const ssize_t n = ::readv(fd, vec, iovcnt);	//readv() 是 POSIX 定义的系统调用，作用是在一次系统调用中将数据读入多个缓冲区（iovec）。
