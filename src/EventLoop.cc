...
/*
EventLoop 的设计是线程绑定的，即每个 EventLoop 实例应该只在一个特定的线程中运行。如果需要多个 EventLoop，可以为每个线程创建一个独立的 EventLoop 实例。
EventLoop运行在不同thread中，不变的东西：
	EventLoop 对象的成员变量和功能逻辑，存储在内存中。
变化的东西：
	线程绑定关系、线程安全性、任务调度行为、Poller 的行为、日志和调试信息。
正确设计：
	每个 EventLoop 实例应该只在一个特定的线程中运行，避免跨线程使用。
*/
 ...
 * eventfd通常用于消息传递，不涉及资源传递，跟pipe不一样
 ...
 */
// 创建wakeupfd 用来notify唤醒subReactor处理新来的channel，因为subReactor可能阻塞在epoll_wait
int createEventfd()
{
...
/*
初始化了looping_（是否正在循环）、quit_（是否退出循环）、callingPendingFunctors_（是否正在调用待执行的任务）等标志。
创建了Poller（用于处理I/O多路复用），并使用createEventfd创建了wakeupFd_（用于线程间唤醒）。
创建了一个Channel来监听wakeupFd_的事件，设置了读事件的回调（handleRead）。
设置了wakeupChannel_来监听wakeupFd_的读取事件。
*/
EventLoop::EventLoop()
    : looping_(false)
    , quit_(false)
    ...
    , wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    wakeupChannel_->enableReading(); // 将channel的听事件设置为感兴趣，并注册到poll中，每一个EventLoop都将监听wakeupChannel_的EPOLL读事件了
}
EventLoop::~EventLoop()
{
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
		...
		//在 muduo 的 EventLoop 中，epoll_wait() 通常是以阻塞模式运行的（即超时时间不为 0，此处为kPollTimeMs=10s），这是为了在没有事件时让线程休眠，避免空转消耗 CPU
...
// 在当前loop中执行cb，cb通常是tcpserver用bind创建的一个可调用实例
...
void EventLoop::doPendingFunctors()
{
    ...
    callingPendingFunctors_ = true;		//设置正在执行回调任务标志

    {
        std::unique_lock<std::mutex> lock(mutex_);
		//将待执行的回调任务从 pendingFunctors_ 移动到 functors 中，避免修改正在执行的回调队列。
