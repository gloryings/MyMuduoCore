...
/*
事件循环线程池，用于高效地管理和调度多个 EventLoop（事件循环）。
它支持多线程模型，并通过一致性哈希策略来分配任务，确保负载均衡和高效的事件处理。
*/
...
public:
    ...
    // 如果工作在多线程中，baseLoop_(mainLoop)会默认以轮询的方式分配Channel给subLoop，此处将其改写为一致性哈希方法将任务分配给各loop，保证负载均衡
...
private:
    ...
    std::vector<std::unique_ptr<EventLoopThread>> threads_;//IO线程的列表,std::unique_ptr → 线程的所有权由线程池管理,用于在析构时自动释放线程对象
