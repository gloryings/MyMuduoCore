...
#include <vector>		//使用 std::vector 来存储事件列表。
#include <atomic>		//使用 std::atomic 保证线程安全的操作。
#include <memory>		//使用 std::unique_ptr 管理指针，防止内存泄漏。
#include <mutex>		//引入互斥锁，用于线程同步。
...
// 事件循环类 主要包含了两个大模块 Channel Poller(epoll的抽象，采用工厂模式)
class EventLoop : noncopyable
{
public:
	//function是一个通用的函数包装器，用于存储、复制和调用任何可调用对象，指定了函数的签名（返回类型和参数类型）
...
private:
    ...
    int wakeupFd_; // 作用：当mainLoop获取一个新用户的Channel 需通过轮询算法选择一个subLoop 通过该eventfd成员唤醒subLoop处理Channel
    std::unique_ptr<Channel> wakeupChannel_;	//用于唤醒该loop的poll，收到mainloop的消息时会立即调用epoll_wait(正常会阻塞10s，避免eventloop不断调用epoll_wait空转，消耗大量cpu资源)，更新active_channellist，然后执行read进行update
