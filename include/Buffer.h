...
#include <stddef.h>
/*
| kCheapPrepend | 已读数据（可变为可写区域） | 可读数据 | 可写数据 |
↑                ↑         					↑         ↑
begin()       readerIndex 				writerIndex buffer_.size()
*/
...
public:
	...
	// 如果读取的数据长度小于可读长度，仅仅移动readerIndex_，代表还有一些数据没读完
	// 如果读取长度等于或超过可读长度，代表缓冲区全部读完，重置缓冲区。
    ...
    // 把onMessage函数上报的Buffer数据 转成string类型的数据返回，用于tcpserver处获取socket中消息至buffer中，同时进行复位
    std::string retrieveAllAsString() { return retrieveAsString(readableBytes()); }
    std::string retrieveAsString(size_t len)
    {
        std::string result(peek(), len);	//可读起始地址和可读长度
    ...
    {
        ensureWritableBytes(len);	//确保有足够的写空间
        std::copy(data, data+len, beginWrite());
        writerIndex_ += len;
    }
    char *beginWrite() { return begin() + writerIndex_; }				//返回可写区域的指针
    const char *beginWrite() const { return begin() + writerIndex_; }	//返回可写区域的常量指针常量
...
private:
    ...
    {
        /**
         * | kCheapPrepend |xxx| reader | writer |                     // xxx标示reader中已读的部分，可将已读部分变成可写部分
